...
{
    KZProvinceModel *pModel;
    KZAddressModel  *aModel;
}
...
@property(nonatomic,assign)BOOL isLevelModel;
...
-(void)setArrayClass:(NSArray *)array{
    ...
    for (id levelTwo in array) {
        ...
        } else if ([levelTwo isKindOfClass:[KZProvinceModel class]]) {
            _isLevelDic=NO;
            _isLevelString=NO;
            _isLevelArray=NO;
            _isLevelModel = YES;
...
-(UIToolbar *)setToolbarStyle{
    ...
    UIButton *leftButton = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, 100, ZHToobarHeight)];
    leftButton.titleLabel.textAlignment = NSTextAlignmentLeft;
    [leftButton setTitleColor:[UIColor colorWithHexString:@"#1D8FE1"] forState:UIControlStateNormal];
    ...
    UIButton *rightButton = [[UIButton alloc]initWithFrame:CGRectMake(kScreenWidth - 100, 0, 100, ZHToobarHeight)];
    rightButton.titleLabel.textAlignment = NSTextAlignmentRight;
    [rightButton setTitleColor:[UIColor colorWithHexString:@"#1D8FE1"] forState:UIControlStateNormal];
...
-(NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component
{
    ...
    if (_isLevelArray) {
        ...
        NSInteger pIndex = [pickerView selectedRowInComponent:0];
        KZProvinceModel *model = _plistArray[pIndex];

        if (0 == component) {
            rowArray = _plistArray;
        } else {
            rowArray = model.cityModelArr;
        }
    }
...
- (NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component{
    ...
    if (_isLevelArray) {
        ...
        NSInteger pIndex = [pickerView selectedRowInComponent:0];
        KZProvinceModel *model1 = _plistArray[row];
        KZProvinceModel *model2 = _plistArray[pIndex];
        
        if (0 == component) {
            rowTitle = model1.areaName;
        } else {
            rowTitle = model2.cityModelArr[row].areaName;
        }
    }
    return rowTitle;
}
        ...
        if (_resultString) {
            ...
            if (_isLevelString) {
              ...
              _resultString=[NSString stringWithFormat:@"%@-%@",_state,_city];
                ...
                pModel = _plistArray[i];
                aModel =  pModel.cityModelArr[j];
                _resultString = [NSString stringWithFormat:@"%@-%@-%@-%@",pModel.areaName,aModel.areaName,pModel.idNum,aModel.idNum];
            }
        }
    }else if (_datePicker) {
