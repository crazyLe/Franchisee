...
- (void)setDigit:(NSUInteger)aDigit from:(NSUInteger)last{
    ...
    int count = 1;
    if (aDigit > last) {
        for (NSUInteger i = last + 1; i < aDigit + 1; ++i) {
            ++count;
            [str appendFormat:@"\n%zd", i];
        }
    } else {
        for (NSUInteger i = last + 1; i < 10; ++i) {
            ++count;
            [str appendFormat:@"\n%zd", i];
        }
        for (NSUInteger i = 0; i < aDigit + 1; ++i) {
            ++count;
            [str appendFormat:@"\n%zd", i];
        }
    }
...
//    CGFloat digitWidth = (allWidth - (self.numberSize + 1) * splitSpaceWidth) / self.numberSize;
    CGFloat digitWidth = 20*kWidthScale;
