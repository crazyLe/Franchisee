...
@interface KZAssociationInfoController ()<UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource>
...
@property (weak, nonatomic) IBOutlet UITextField *associationTF;
...
@implementation KZAssociationInfoController
...
- (void)createUI {
    ...
    [self setTextfiledValue:_associationTF];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    return tableView.tag==10?_provinceArr.count:tableView.tag==20?_cityArr.count:tableView.tag==30?_schoolArr.count:0;
}
...
- (IBAction)confirmBtnClick:(id)sender {
    ...
    if (_associationTF.text.length == 0) {
        [self.hudManager showErrorSVHudWithTitle:@"请填写社团名称" hideAfterDelay:1.0];
        return;
    }
    ...
    paramDict[@"community_name"] = _associationTF.text;
